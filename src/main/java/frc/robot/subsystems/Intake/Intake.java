...
public class Intake extends SubsystemBase {
	...
	private static final LoggedTunableNumber rollerTargetAccelerationConfig = new LoggedTunableNumber(
	        "Intake/Roller/Acceleration");
	private static final LoggedTunableNumber extenderMaxVelocityConfig = new LoggedTunableNumber(
	        "Intake/Extender/MaxVel");
	private static final LoggedTunableNumber extenderTargetAccelerationConfig = new LoggedTunableNumber(
	        "Intake/Extender/TargetAcceleration");
	...
	public void periodic() {
		...
		int hc = hashCode();
		if (eKP.hasChanged(hc) || eKD.hasChanged(hc) || extenderMaxVelocityConfig.hasChanged(hc)
		        || extenderTargetAccelerationConfig.hasChanged(hc))
			configExtender();
		if (rKP.hasChanged(hc) || rKD.hasChanged(hc) || rKV.hasChanged(hc)
		        || rollerTargetAccelerationConfig.hasChanged(hc))
			configRoller();
	...
	// Returns true if extender is out at least 90 degrees from zero, returns false
	// if else
	public boolean checkExtenderPosition() {
		return getExtenderPos().getDegrees() > Constants.IntakeConstants.ExtenderConstants.MIN_REQ_EXTENDER_ANGLE
		        .getDegrees();
	}
