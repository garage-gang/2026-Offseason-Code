...
public class Shooter extends SubsystemBase {
	...
	private static final LoggedTunableNumber wheelTargetAccelerationConfig = new LoggedTunableNumber(
	        "shooter/Wheel/Acceleration");
	private static final LoggedTunableNumber hoodMaxVelocityConfig = new LoggedTunableNumber("shooter/Hood/MaxVel");
	private static final LoggedTunableNumber hoodTargetAccelerationConfig = new LoggedTunableNumber(
	        "shooter/Hood/TargetAcceleration");
	private static final LoggedTunableNumber rotationMaxVelocityConfig = new LoggedTunableNumber(
	        "shooter/Rotation/MaxVel");
	private static final LoggedTunableNumber rotationTargetAccelerationConfig = new LoggedTunableNumber(
	        "shooter/Rotation/TargetAcceleration");
	...
	public void periodic() {
		...
		int hc = hashCode();
		if (wKV.hasChanged(hc) || wKP.hasChanged(hc) || wKI.hasChanged(hc)
		        || wheelTargetAccelerationConfig.hasChanged(hc))
			configWheel();
		if (hKP.hasChanged(hc) || hKI.hasChanged(hc) || hKD.hasChanged(hc) || hoodMaxVelocityConfig.hasChanged(hc)
		        || hoodTargetAccelerationConfig.hasChanged(hc))
			configHood();
		if (rKP.hasChanged(hc) || rKD.hasChanged(hc) || rotationMaxVelocityConfig.hasChanged(hc)
		        || rotationTargetAccelerationConfig.hasChanged(hc))
			configRotation();
